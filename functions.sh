...
spot()
{
  mdfind "kMDItemFSName=='*$(echo $@ | sed -E 's/[ ]+/*/g')*'cd"
}
...
comm_fast()
{
  awk 'ARGIND==1 { _[$0]=1; next } !($0 in _) || _[$0]==0 { next } { print; _[$0]=0 }' $*
}
...
union_fast()
{
  awk 'ARGIND==1 { _[$0]=1; print; next } !($0 in _) { print }' $*
}

# print rows found only of the left side file
lhs_only_fast()
{
  awk 'ARGIND==1 { _[$0]=1; next } $0 in _ { _[$0]=0 } END { for (k in _) if (_[k]) print k }' $*
}

# print rows only found on the right side file
rhs_only_fast()
{
  awk 'ARGIND==1 { _[$0]=0; next } !($0 in _) { print; _[$0]=0 }' $*
}

# print the rows which are found in one file but not in the other
diff_fast()
{
  awk 'ARGIND==1 { _[$0]=1; next } !($0 in _) { print } { _[$0]=0 } END { for (k in _) if (_[k]) print k }' $*
}

# print a histogram of the rows (desc sorted count)
freq_fast()
{
  awk '{ ++_[$0] } END { PROCINFO["sorted_in"]="@val_num_desc"; for (k in _) print _[k], k }' $*
}
hist_fast=freq_fast

nunique_fast()
{
  awk '{ ++_[$0] } END { print length(_) }' $*
}

# histogram of one field by another (whitespace version)
histby()
{
  awk "{ d[\$$1]+= \$$2 } END { for (k in d) print d[k] \"\t\" k }" | sort -nr
}

# histogram of one field by another (naive csv version)
histbyc()
{
  awk -F, "{ d[\$$1]+= \$$2 } END { for (k in d) print d[k] \"\t\" k }" | sort -nr
}
#export -f histbyc
